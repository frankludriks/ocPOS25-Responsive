...
$posonly = "";
if (isset($_GET['posonly']) && $_GET['posonly'] == 1) {
	$posonly = "o.in_store_purchase = '1' AND ";
}

$Q_Order = mysql_query("SELECT o.orders_id,payment_method,o.date_purchased FROM " . ORDERS . " o, " . ORDERS_TOTAL . " ot 
							WHERE " . $posonly . "
							...
							o.void = '0' AND
							ot.class = 'ot_total' ORDER BY o.date_purchased");

while($R_Order = mysql_fetch_assoc($Q_Order)){
	$Index = date("w",strtotime($R_Order['date_purchased']));
...
for ($i=0; $i<14; $i+=2){
	$y=750;
	$j=$D->ScreenX($i);
	$D->Bar($j-15, $D->ScreenY($T_GrandTotal[$i/2]), $j+15, $D->ScreenY(0), "#5BC558", $Day[$i/2], "#000000");
	
	//$j=$D->ScreenX($i+1.0);
	//$D->Bar($j-15, $D->ScreenY($T_Gross[$i/2]), $j+15, $D->ScreenY(0), "#FFB400", $Day[$i/2], "#000000");

}

ImagePng($D->Img, "graphs/bar_days.png");
ImageDestroy($D->Img);
...
<table width="100%" border="0" cellpadding="2" cellspacing="0" align="center">
 ...
 <td width="100%" class="tdBlue" colspan="8" align="center">
  <b>
  <?php echo DAY_WEEK_REPORT2; ?>
  </b>
 </td>
 </tr>
 <tr>
 <td width="100%" colspan="8" align="center">
 <img src="graphs/bar_days.png?rand=<?php echo(rand()); ?>" border="0">
 </td>
 </tr>
 <tr>
 <td width="16%" class="tdBlue" align="center"><b><?php echo DAY; ?></b></td>
...
?>
 <tr>
 <td width="16%" class="tdBlue" align="center"><b><?php echo(date("l",mktime(0,0,0,1,4+$i,2004))); ?></b></td>
 <td width="14%" align="right"><?php echo($T_Num[$i]); ?></td>
...
<?php for($i = 0; $i < 7; $i++){ ?>
 ...
 <td width="100%" class="tdBlue" colspan="8" align="center">
  <b>
  Payment Type Report - <?php echo(date("l",mktime(0,0,0,1,4+$i,2004))); ?>
  </b>
 </td>
 </tr>
 <tr>
